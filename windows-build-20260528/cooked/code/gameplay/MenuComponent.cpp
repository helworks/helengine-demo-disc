...
std::string MenuComponent::SerializedComponentTypeId = "city.menu.MenuComponent, gameplay";
...
{
for (int32_t itemIndex = 0; itemIndex < panelRuntime->get_Items()->get_Length(); itemIndex++) {
...
void MenuComponent::ConfirmSelection(Keys key)
{
    ...
    {
return;    }
    if (this->ActivePanel->get_SelectedItemIndex() < 0 || this->ActivePanel->get_SelectedItemIndex() >= this->ActivePanel->get_Items()->get_Length())
    {
return;    }
...
float3 position = runtimeItem->get_OwnerEntity()->get_Position();
...
throw new ArgumentNullException("panelRuntime");
    ...
    if (selectedItemIndex < 0 || selectedItemIndex >= panelRuntime->get_Items()->get_Length())
    {
throw ([&]() {
...
return;    }
    ...
    if (itemComponent->get_ActionKind() == MenuActionKind::OpenPanel)
    {
this->ActivatePanel(itemComponent->get_TargetId(), true);
    }
else {
    if (itemComponent->get_ActionKind() == MenuActionKind::LoadScene)
    {
this->LoadScene(itemComponent->get_TargetId());
    }
else {
    if (itemComponent->get_ActionKind() == MenuActionKind::Back)
    {
this->NavigateBack();
    }
else {
...
{
    if (this->ActivePanel == nullptr || this->ActivePanel->get_Items()->get_Length() == 0)
    {
return;    }
    if (delta == 0)
    {
...
nextIndex += delta;
    if (nextIndex < 0)
    {
nextIndex = this->ActivePanel->get_Items()->get_Length() - 1;
    }
else {
    if (nextIndex >= this->ActivePanel->get_Items()->get_Length())
    {
nextIndex = 0;
    }
}
...
return;    }
panelRuntime->get_ItemsScrollComponent()->ScrollOffsetChanged -= &MenuComponent::HandleItemsScrollOffsetChanged;
this->ReleaseItemRuntimes(panelRuntime->get_Items());
}

void MenuComponent::ReleasePanelRuntimes()
{
for (int32_t panelIndex = 0; panelIndex < this->PanelRuntimes->Count(); panelIndex++) {
this->ReleasePanelRuntime((*this->PanelRuntimes)[panelIndex]);
}
}
...
{
    if (panelRuntime->get_SelectedItemIndex() >= 0 && panelRuntime->get_SelectedItemIndex() < panelRuntime->get_Items()->get_Length())
    {
...
throw new ArgumentNullException("panelRuntime");
    ...
    if (itemIndex < 0 || itemIndex >= panelRuntime->get_Items()->get_Length())
    {
throw ([&]() {
...
for (int32_t index = 0; index < panelRuntime->get_Items()->get_Length(); index++) {
