...
std::string NintendoDsReturnOverlayComponent::MainMenuSceneId = "DemoDiscMainMenu";
...
for (int32_t componentIndex = 0; componentIndex < this->get_Parent()->get_Components()->get_Count(); componentIndex++) {
    InteractableComponent* interactable = he_cpp_try_cast<InteractableComponent>((*this->get_Parent()->get_Components())[componentIndex]);
