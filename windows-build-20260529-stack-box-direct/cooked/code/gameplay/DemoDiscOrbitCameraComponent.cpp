...
double DemoDiscOrbitCameraComponent::GamepadDeadzone = 0.18;

double DemoDiscOrbitCameraComponent::MinimumOrbitRadius = 0.001;
...
Parent->set_LocalPosition(float3(static_cast<float>(x), static_cast<float>(y), static_cast<float>(z)));
float4 orientation;
float4::CreateFromYawPitchRoll(this->CurrentYawRadians, this->CurrentPitchRadians, 0.0f, orientation);
