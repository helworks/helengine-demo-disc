...
namespace city.tests {
    ...
    public sealed class TestGeneratedAssetGraph : IDisposable {
        ...
        readonly List<IEditorProjectAuthoringSession> Sessions = new List<IEditorProjectAuthoringSession>();
        ...
        public TestGeneratedAssetGraph(string projectRootPath) {
            string root = Path.GetFullPath(projectRootPath ?? throw new ArgumentNullException(nameof(projectRootPath)));
            Directory.CreateDirectory(Path.Combine(root, "assets"));
            CoreValue = new EditorCore(new Project { Name = "rendering-test", Path = root });
            CoreValue.Initialize(new RenderingTestRenderManager3D(), new RenderingTestRenderManager2D(), null, new PlatformInfo("test", "test-version"));
            ...
            ShaderLibrary = new EditorBuiltInShaderAssetLibrary(new ShaderBackendRegistry());
        ...
        public IEditorProjectAuthoringSession CreateAuthoringSession(string projectRootPath) {
            ...
            Directory.CreateDirectory(Path.Combine(root, "assets"));
            ContentManager contentManager = new ContentManager(new HostFileSystemContentStreamSource(Path.Combine(root, "assets")));
            try {
        ...
        public void Dispose() {
            ...
            for (int index = Sessions.Count - 1; index >= 0; index--) {
                DisposeOne(Sessions[index], failures);
            }
        ...
        static void DisposeOne(IDisposable disposable, List<Exception> failures) {
            try { disposable?.Dispose(); } catch (Exception exception) { failures.Add(exception); }
        }
    }

    sealed class RenderingTestRenderManager3D : RenderManager3D {
        public override RuntimeModel BuildModelFromRaw(ModelAsset data) {
            if (data == null) throw new ArgumentNullException(nameof(data));
            RenderingTestRuntimeModel model = new RenderingTestRuntimeModel();
            model.SetBounds(data.BoundsMin, data.BoundsMax);
            return model;
        }
    }

    sealed class RenderingTestRuntimeModel : RuntimeModel { }

    sealed class RenderingTestRenderManager2D : RenderManager2D {
    ...
    sealed class RenderingTestRuntimeTexture : RuntimeTexture { }
}
