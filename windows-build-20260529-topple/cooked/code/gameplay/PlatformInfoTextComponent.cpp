...
auto __ctor_arg_0000009C = static_cast<int32_t>(Math::Ceiling(measuredSize.X * fontScale));
auto __ctor_arg_0000009D = static_cast<int32_t>(Math::Ceiling(measuredSize.Y * fontScale));
return int2(__ctor_arg_0000009C, __ctor_arg_0000009D);
})());
...
Entity *childEntity = (*parentEntity->get_Children())[childIndex];
    if (childEntity == nullptr)
    {
continue;
    }
TextComponent* textComponent;
    if (this->TryFindTextComponent(childEntity, textComponent))
    {
...
for (int32_t index = 0; index < entity->get_Components()->get_Count(); index++) {
    TextComponent* textComponent = he_cpp_try_cast<TextComponent>((*entity->get_Components())[index]);
    if (textComponent != nullptr)
    {
...
this->CollectChildTextEntities(entity, textEntities);
    ...
    {
return false;    }
this->PlatformNameTextEntity = (*textEntities)[0];
this->PlatformVersionTextEntity = (*textEntities)[1];
...
bool PlatformInfoTextComponent::TryFindTextComponent(Entity* entity, TextComponent*& textComponent)
{
    if (entity == nullptr)
    {
throw new ArgumentNullException("entity");
...
for (int32_t index = 0; index < entity->get_Components()->get_Count(); index++) {
    TextComponent* foundTextComponent = he_cpp_try_cast<TextComponent>((*entity->get_Components())[index]);
