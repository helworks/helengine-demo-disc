...
The project now has a generated control-icon pack under `assets/textures/instructions/controls/generated`, with one folder per platform family and one PNG per control name. Rendering and physics scene generators do not currently have a reusable way to consume those assets.
...
- load `assets/textures/instructions/controls/generated/manifest.json`
