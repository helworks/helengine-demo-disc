# Render Motion Probe Scene Implementation Plan
...
- `C:\dev\helprojs\city\assets\codebase\rendering\RenderMotionProbeComponent.cs`
...
- `C:\dev\helprojs\city\assets\codebase\physics.tools\PhysicsSceneCatalog.cs`
  Exposes the new `test_scene_render_motion_probe` scene id and inserts it into the stable generated-physics scene order.
- `C:\dev\helprojs\city\assets\codebase\physics.tools\PhysicsSceneFactory.cs`
  Routes the new scene id, authors the static reference cubes plus fixed camera, and creates the hero cube entity with `RenderMotionProbeComponent`.
...
### Task 1: Add Failing Source Coverage For The New Motion Probe Scene
...
```powershell
rtk dotnet test C:\dev\helworks\helengine\engine\helengine.editor.tests\helengine.editor.tests.csproj --filter FullyQualifiedName~City_render_motion_probe_scene_source_is_exposed_as_visual_only_with_motion_probe_component -v minimal
```

Expected: `FAIL` because the catalog constant, factory method, and motion component usage do not exist yet.
...
### Task 2: Add The Motion Probe Runtime Component

**Files:**
- Create: `C:\dev\helprojs\city\assets\codebase\rendering\RenderMotionProbeComponent.cs`
...
### Task 3: Author The Render-Only Motion Probe Scene
...
### Task 4: Boot Windows Directly Into The Motion Probe And Validate It
...
```json
"selectedSceneIds": [
  "test_scene_render_motion_probe"
],
"sceneOrders": [
  {
    "sceneId": "test_scene_render_motion_probe",
    "orderNumber": 1
  }
]
```
...
Expected: `C:\dev\helprojs\city\windows-build\helengine_windows.exe` exists and `C:\dev\helprojs\city\windows-build\cooked\scenes\physics\test_scene_render_motion_probe.hasset` exists.
...
Expected: runtime boots directly into `test_scene_render_motion_probe`.
...
```text
[Host] Loading startup scene from runtime scene catalog entry 'test_scene_render_motion_probe'.
```
